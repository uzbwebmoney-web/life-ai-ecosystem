...
def test_unlock_session_ttl():
    lock_vault(42)
    assert not is_vault_unlocked(42)
    unlock_vault(42, ttl_seconds=1)
    assert is_vault_unlocked(42)
    time.sleep(1.1)
    assert not is_vault_unlocked(42)
...
def test_lock_vault_on_menu_exit():
    ...
    unlock_vault(5)
    assert is_vault_unlocked(5)
    lock_vault_on_menu_exit(user)
    assert not is_vault_unlocked(5)
    lock_vault_on_menu_exit(User(id=6, telegram_id=6))
    assert not is_vault_unlocked(6)
