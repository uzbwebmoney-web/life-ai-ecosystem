...
def vault_item_open_kb(submodule_id: str, record: LifeRecord, lang: str = "ru") -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    if not record_has_file(record):
        rows.append(
            [
                InlineKeyboardButton(
                    text=t(lang, "vlt_attach_file_btn"),
                    callback_data=f"vlt:attach:{submodule_id}:{record.id}",
                )
            ]
        )
    rows.append(
        [InlineKeyboardButton(text=t(lang, "btn_back_module"), callback_data=f"sub:vault:{submodule_id}")]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def vault_delete_confirm_kb(submodule_id: str, record_id: int, lang: str = "ru") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=t(lang, "vlt_confirm_yes"),
                    callback_data=f"vlt:delok:{submodule_id}:{record_id}",
                ),
